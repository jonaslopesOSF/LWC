...
function validateAccessorDecoratedWithApi(Ctor, fieldName, descriptor) {
  if (process.env.NODE_ENV !== 'production') {
    if (isUndefined$1(descriptor)) {
      ...
      assert.isTrue(isFunction$1(descriptor.get), `Compiler Error: Missing getter for property ${toString(fieldName)} decorated with @api in ${Ctor}. You cannot have a setter without the corresponding getter.`);
...
function evaluateTemplate(vm, html) {
  if (process.env.NODE_ENV !== 'production') {
    assert.isTrue(isFunction$1(html), `evaluateTemplate() second argument must be an imported template instead of ${toString(html)}`);
  }
...
const PROPERTY_DELIMITER = /:(.+)/; // Implementation of the CSS property to IDL attribute algorithm.
...
function idlAttributeToCssProperty(attribute) {
  let output = '';
...
function styleTextToCssDeclaration(cssText) {
...
function cssDeclarationToStyleText(styleDeclaration) {
  return Object.entries(styleDeclaration).map(([prop, value]) => {
...
const renderer = {
  ...
  getProperty(node, key) {
    ...
    if (node.type === HostNodeType.Element) {
      const attrName = getAttrNameFromPropName(key); // Handle all the boolean properties.
  ...
  getStyleDeclaration(element) {
    function getStyleAttribute() {
      return element.attributes.find(attr => attr.name === 'style' && isNull(attr.namespace));
    }
